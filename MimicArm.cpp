...
)
{
	...
	//serialPort.Open(3);
	//Sleep(10000);
	//DPRINT("Is connected = " << serialPort.IsOpened() << '\n');
	//if (serialPort.IsOpened()) {
	//	//int a[] = { 1, 0 };
	//	//DPRINT("bytes sent succesfully" << serialPort.SendData((char *) a, sizeof(a)) << '\n');
	//	//char newline[] = { '\n' };
	//	//DPRINT("newline " << serialPort.SendData(newline, sizeof(newline)) << '\n');
	//	{
	//		char w[] = "1 1000\n";
	//		serialPort.SendData(w, sizeof(w));
	//	}
	//	Sleep(1000);
	//	{
	//		char w[] = "1 999\n";
	...
	//System::IO::Ports::SerialPort serialPort(L"COM3", 9600);
	//serialPort.Open();
	//array<int>^ a = gcnew array < int > {1, 1000};
	//serialPort.Write((array<wchar_t>^) a, 0, 8);
	//serialPort.Write(L"\n");
	////serialPort.Write("1,1000\n");
	//serialPort.Close();
...
int CBodyBasics::Run(HINSTANCE hInstance, int nCmdShow)
{
    ...
    wc.lpszClassName = "BodyBasicsAppDlgWndClass";
    ...
    if (m_hWnd)
    {
        ...
        if (SUCCEEDED(hr) && m_pRenderTarget && m_pCoordinateMapper)
        {
            ...
            for (int i = 0; i < nBodyCount; ++i)
            {
                ...
                if (pBody)
                {
                    ...
                    if (SUCCEEDED(hr) && bTracked)
                    {
						...
						if (SUCCEEDED(hr)) {
							DPRINT("Tracking ID " << trackingId << '\n');
							if (currentTrackingId == 0 && trackingId != 0) {
			...
			if (currentBody) {
				...
				currentBody->get_HandLeftState(&leftHandState);
				currentBody->get_HandRightState(&rightHandState);
				HRESULT hr2 = currentBody->GetJointOrientations(JointType_Count, jointOrientations);

				hr = currentBody->GetJoints(_countof(joints), joints);
				if (SUCCEEDED(hr) && SUCCEEDED(hr2))
				{
					...
					DrawBody(joints, jointPoints, jointOrientations);
...
bool CBodyBasics::SetStatusMessage(_In_z_ WCHAR* szMessage, DWORD nShowTimeMsec, bool bForce)
{
    ...
    {
        SetDlgItemTextW(m_hWnd, IDC_STATUS, szMessage);
...
void CBodyBasics::DrawBody(const Joint* pJoints, const D2D1_POINT_2F* pJointPoints, const JointOrientation* pJointOrientation)
{
	if (GetAsyncKeyState(VK_SPACE) & 0x8000) {
		// Space bar held. Do not send commands
	} else {
		// Space Bar not held 
		arm.sendPositions(pJoints, pJointOrientation);
	}
